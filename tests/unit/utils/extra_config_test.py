...
def test_load_extra_model_paths_expands_appdata(
    ...
    for actual_call, expected_call in zip(mock_add_model_folder_path.call_args_list, expected_calls):
        for i in range(len(actual_call.args)):
            assert Path(actual_call.args[i]) == Path(expected_call[i])
...
def test_load_extra_path_config_relative_base_path(
    ...
    expected_checkpoints = (tmp_path / sub_folder / "checkpoints").resolve()
    expected_some_value = (tmp_path / sub_folder / "some_value").resolve()
    ...
    assert Path(actual_paths[0]) == Path(expected_checkpoints)
    ...
    assert Path(actual_paths[0]) == Path(expected_some_value)
