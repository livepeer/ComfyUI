...
from hiddenswitch_comfy.ldm.modules.attention import optimized_attention
from hiddenswitch_comfy.ldm.flux.layers import EmbedND
from hiddenswitch_comfy.ldm.flux.math import apply_rope
from hiddenswitch_comfy.ldm.modules.diffusionmodules.mmdit import RMSNorm
import hiddenswitch_comfy.ldm.common_dit
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        e = (hiddenswitch_comfy.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e).chunk(6, dim=1)
...
class Head(nn.Module):
    ...
    def forward(self, x, e):
        ...
        """
        # assert e.dtype == torch.float32
        e = (hiddenswitch_comfy.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e.unsqueeze(1)).chunk(2, dim=1)
...
class MLPProj(torch.nn.Module):
    ...
    def forward(self, image_embeds):
        if self.emb_pos is not None:
            image_embeds = image_embeds[:, :self.emb_pos.shape[1]] + hiddenswitch_comfy.model_management.cast_to(self.emb_pos[:, :image_embeds.shape[1]], dtype=image_embeds.dtype, device=image_embeds.device)
...
class WanModel(torch.nn.Module):
    ...
    def forward(self, x, timestep, context, clip_fea=None, transformer_options={}, **kwargs):
        bs, c, t, h, w = x.shape
        x = hiddenswitch_comfy.ldm.common_dit.pad_to_patch_size(x, self.patch_size)
