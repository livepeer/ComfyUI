...
from hiddenswitch_comfy import sd1_clip
from hiddenswitch_comfy import sdxl_clip
...
class HiDreamTEModel(torch.nn.Module):
    def __init__(self, clip_l=True, clip_g=True, t5=True, llama=True, dtype_t5=None, dtype_llama=None, device="cpu", dtype=None, model_options={}):
        ...
        if t5:
            dtype_t5 = hiddenswitch_comfy.model_management.pick_weight_dtype(dtype_t5, dtype, device)
        ...
        if llama:
            dtype_llama = hiddenswitch_comfy.model_management.pick_weight_dtype(dtype_llama, dtype, device)
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            if self.clip_l is not None:
                ...
                l_pooled = torch.zeros((1, 768), device=hiddenswitch_comfy.model_management.intermediate_device())

            if self.clip_g is not None:
                g_out, g_pooled = self.clip_g.encode_token_weights(token_weight_pairs_g)
            else:
                g_pooled = torch.zeros((1, 1280), device=hiddenswitch_comfy.model_management.intermediate_device())
        ...
        if t5_out is None:
            t5_out = torch.zeros((1, 128, 4096), device=hiddenswitch_comfy.model_management.intermediate_device())

        if ll_out is None:
            ll_out = torch.zeros((1, 32, 1, 4096), device=hiddenswitch_comfy.model_management.intermediate_device())

        if pooled is None:
            pooled = torch.zeros((1, 768 + 1280), device=hiddenswitch_comfy.model_management.intermediate_device())
