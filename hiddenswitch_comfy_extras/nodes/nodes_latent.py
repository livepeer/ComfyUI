...
from hiddenswitch_comfy.component_model.tensor_types import Latent
...
def reshape_latent_to(target_shape, latent, repeat_batch=True):
    if latent.shape[1:] != target_shape[1:]:
        latent = hiddenswitch_comfy.utils.common_upscale(latent, target_shape[-1], target_shape[-2], "bilinear", "center")
    if repeat_batch:
        return hiddenswitch_comfy.utils.repeat_to_batch_size(latent, target_shape[0])
    else:
        return latent
...
class LatentAddNoiseChannels:
    ...
    def inject_noise(self, samples: Latent, std_dev, seed: int, slice_i: int, slice_j: int):
        ...
        latent = samples["samples"]
        with hiddenswitch_comfy.utils.seed_for_block(seed):
