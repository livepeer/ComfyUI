import hiddenswitch_comfy.sampler_names
from hiddenswitch_comfy import samplers
from hiddenswitch_comfy import model_management
from hiddenswitch_comfy import sample
from hiddenswitch_comfy.execution_context import current_execution_context
from hiddenswitch_comfy.k_diffusion import sampling as k_diffusion_sampling
from hiddenswitch_comfy.cmd import latent_preview
import torch
from hiddenswitch_comfy import utils
from hiddenswitch_comfy import node_helpers
from hiddenswitch_comfy.nodes.package_typing import Seed, Seed64
from hiddenswitch_comfy.samplers import KSAMPLER
...
class BasicScheduler:
    ...
    def INPUT_TYPES(s):
        return {"required":
                    {"model": ("MODEL",),
                     "scheduler": (hiddenswitch_comfy.sampler_names.SCHEDULER_NAMES,),
...
class BetaSamplingScheduler:
    ...
    def get_sigmas(self, model, steps, alpha, beta):
        sigmas = hiddenswitch_comfy.samplers.beta_scheduler(model.get_model_object("model_sampling"), steps, alpha=alpha, beta=beta)
        return (sigmas, )
...
class KSamplerSelect:
    ...
    def INPUT_TYPES(s):
        return {"required":
                    {"sampler_name": (hiddenswitch_comfy.sampler_names.SAMPLER_NAMES,),
                      }
               }
...
class SamplerDPMPP_3M_SDE:
    ...
    def get_sampler(self, eta, s_noise, noise_device):
        ...
        sampler = hiddenswitch_comfy.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerDPMPP_2S_Ancestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = hiddenswitch_comfy.samplers.ksampler("dpmpp_2s_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerEulerAncestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = hiddenswitch_comfy.samplers.ksampler("euler_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerEulerAncestralCFGPP:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = hiddenswitch_comfy.samplers.ksampler(
...
class SamplerLMS:
    ...
    def get_sampler(self, order):
        sampler = hiddenswitch_comfy.samplers.ksampler("lms", {"order": order})
        return (sampler, )
...
class SamplerDPMAdaptative:
    ...
    def get_sampler(self, order, rtol, atol, h_init, pcoeff, icoeff, dcoeff, accept_safety, eta, s_noise):
        sampler = hiddenswitch_comfy.samplers.ksampler("dpm_adaptive", {"order": order, "rtol": rtol, "atol": atol, "h_init": h_init, "pcoeff": pcoeff,
...
class Noise_RandomNoise:
    ...
    def generate_noise(self, input_latent):
        ...
        return hiddenswitch_comfy.sample.prepare_noise(latent_image, self.seed, batch_inds)
...
class SamplerCustom:
    ...
    def sample(self, model, add_noise, noise_seed, cfg, positive, negative, sampler, sigmas, latent_image):
        ...
        latent = latent.copy()
        latent_image = hiddenswitch_comfy.sample.fix_empty_latent_channels(model, latent_image)
...
class Guider_Basic(hiddenswitch_comfy.samplers.CFGGuider):
...
class CFGGuider:
    ...
    def get_guider(self, model, positive, negative, cfg):
        guider = hiddenswitch_comfy.samplers.CFGGuider(model)
        guider.set_conds(positive, negative)
        guider.set_cfg(cfg)
        return (guider,)

class Guider_DualCFG(hiddenswitch_comfy.samplers.CFGGuider):
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None):
        ...
        out = hiddenswitch_comfy.samplers.calc_cond_batch(self.inner_model, [negative_cond, middle_cond, self.conds.get("positive", None)], x, timestep, model_options)
        return hiddenswitch_comfy.samplers.cfg_function(self.inner_model, out[1], out[0], self.cfg2, x, timestep, model_options=model_options, cond=middle_cond, uncond=negative_cond) + (out[2] - out[1]) * self.cfg1
...
class SamplerCustomAdvanced:
    ...
    def sample(self, noise, guider: hiddenswitch_comfy.samplers.CFGGuider, sampler: KSAMPLER, sigmas, latent_image):
        latent = latent_image
        latent_image = latent["samples"]
        latent = latent.copy()
        latent_image = hiddenswitch_comfy.sample.fix_empty_latent_channels(guider.model_patcher, latent_image)
        latent["samples"] = latent_image
        ...
        samples = samples.to(hiddenswitch_comfy.model_management.intermediate_device())
