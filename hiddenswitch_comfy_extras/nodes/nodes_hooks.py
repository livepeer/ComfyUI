...
if TYPE_CHECKING:
    from hiddenswitch_comfy.sd import CLIP

import hiddenswitch_comfy.hooks
import hiddenswitch_comfy.sd
import hiddenswitch_comfy.utils
from hiddenswitch_comfy.cmd import folder_paths
from hiddenswitch_comfy.model_downloader import get_or_download, get_filename_list_with_downloadable
...
class PairConditioningSetProperties:
    ...
    def set_properties(self, positive_NEW, negative_NEW,
                       ...
                       mask: torch.Tensor=None, hooks: hiddenswitch_comfy.hooks.HookGroup=None, timesteps: tuple=None):
        final_positive, final_negative = hiddenswitch_comfy.hooks.set_conds_props(conds=[positive_NEW, negative_NEW],
...
class PairConditioningSetPropertiesAndCombine:
    ...
    def set_properties(self, positive, negative, positive_NEW, negative_NEW,
                       strength: float, set_cond_area: str,
                       mask: torch.Tensor=None, hooks: hiddenswitch_comfy.hooks.HookGroup=None, timesteps: tuple=None):
        final_positive, final_negative = hiddenswitch_comfy.hooks.set_conds_props_and_combine(conds=[positive, negative], new_conds=[positive_NEW, negative_NEW],
                                                                                strength=strength, set_cond_area=set_cond_area,
                                                                                mask=mask, hooks=hooks, timesteps_range=timesteps)
        return (final_positive, final_negative)
...
class ConditioningSetProperties:
    ...
    def set_properties(self, cond_NEW,
                       strength: float, set_cond_area: str,
                       mask: torch.Tensor=None, hooks: hiddenswitch_comfy.hooks.HookGroup=None, timesteps: tuple=None):
        (final_cond,) = hiddenswitch_comfy.hooks.set_conds_props(conds=[cond_NEW],
                                                   strength=strength, set_cond_area=set_cond_area,
                                                   mask=mask, hooks=hooks, timesteps_range=timesteps)
        return (final_cond,)
...
class ConditioningSetPropertiesAndCombine:
    ...
    def set_properties(self, cond, cond_NEW,
                       strength: float, set_cond_area: str,
                       mask: torch.Tensor=None, hooks: hiddenswitch_comfy.hooks.HookGroup=None, timesteps: tuple=None):
        (final_cond,) = hiddenswitch_comfy.hooks.set_conds_props_and_combine(conds=[cond], new_conds=[cond_NEW],
                                                               strength=strength, set_cond_area=set_cond_area,
                                                               mask=mask, hooks=hooks, timesteps_range=timesteps)
        return (final_cond,)
...
class PairConditioningCombine:
    ...
    def combine(self, positive_A, negative_A, positive_B, negative_B):
        final_positive, final_negative = hiddenswitch_comfy.hooks.set_conds_props_and_combine(conds=[positive_A, negative_A], new_conds=[positive_B, negative_B],)
        return (final_positive, final_negative,)
...
class PairConditioningSetDefaultAndCombine:
    ...
    def set_default_and_combine(self, positive, negative, positive_DEFAULT, negative_DEFAULT,
                                hooks: hiddenswitch_comfy.hooks.HookGroup=None):
        final_positive, final_negative = hiddenswitch_comfy.hooks.set_default_conds_and_combine(conds=[positive, negative], new_conds=[positive_DEFAULT, negative_DEFAULT],
                                                                                   hooks=hooks)
        return (final_positive, final_negative)
...
class ConditioningSetDefaultAndCombine:
    ...
    def set_default_and_combine(self, cond, cond_DEFAULT,
                                hooks: hiddenswitch_comfy.hooks.HookGroup=None):
        (final_conditioning,) = hiddenswitch_comfy.hooks.set_default_conds_and_combine(conds=[cond], new_conds=[cond_DEFAULT],
                                                                        hooks=hooks)
...
class SetClipHooks:
    ...
    def apply_hooks(self, clip: CLIP, schedule_clip: bool, apply_to_conds: bool, hooks: hiddenswitch_comfy.hooks.HookGroup=None):
        if hooks is not None:
            ...
            clip.patcher.register_all_hook_patches(hooks, hiddenswitch_comfy.hooks.create_target_dict(hiddenswitch_comfy.hooks.EnumWeightTarget.Clip))
        return (clip,)
...
class CreateHookLora:
    ...
    def create_hook(self, lora_name: str, strength_model: float, strength_clip: float, prev_hooks: hiddenswitch_comfy.hooks.HookGroup=None):
        if prev_hooks is None:
            prev_hooks = hiddenswitch_comfy.hooks.HookGroup()
        prev_hooks.clone()
        ...
        if lora is None:
            lora = hiddenswitch_comfy.utils.load_torch_file(lora_path, safe_load=True)
            self.loaded_lora = (lora_path, lora)

        hooks = hiddenswitch_comfy.hooks.create_hook_lora(lora=lora, strength_model=strength_model, strength_clip=strength_clip)
...
class CreateHookLoraModelOnly(CreateHookLora):
    ...
    def create_hook_model_only(self, lora_name: str, strength_model: float, prev_hooks: hiddenswitch_comfy.hooks.HookGroup=None):
...
class CreateHookModelAsLora:
    ...
    def create_hook(self, ckpt_name: str, strength_model: float, strength_clip: float,
                    prev_hooks: hiddenswitch_comfy.hooks.HookGroup=None):
        if prev_hooks is None:
            prev_hooks = hiddenswitch_comfy.hooks.HookGroup()
        prev_hooks.clone()
        ...
        if weights_model is None:
            out = hiddenswitch_comfy.sd.load_checkpoint_guess_config(ckpt_path, output_vae=True, output_clip=True, embedding_directory=folder_paths.get_folder_paths("embeddings"))
            weights_model = hiddenswitch_comfy.hooks.get_patch_weights_from_model(out[0])
            weights_clip = hiddenswitch_comfy.hooks.get_patch_weights_from_model(out[1].patcher if out[1] else out[1])
            self.loaded_weights = (ckpt_path, weights_model, weights_clip)

        hooks = hiddenswitch_comfy.hooks.create_hook_model_as_lora(weights_model=weights_model, weights_clip=weights_clip,
...
class SetHookKeyframes:
    ...
    def set_hook_keyframes(self, hooks: hiddenswitch_comfy.hooks.HookGroup, hook_kf: hiddenswitch_comfy.hooks.HookKeyframeGroup=None):
...
class CreateHookKeyframe:
    ...
    def create_hook_keyframe(self, strength_mult: float, start_percent: float, prev_hook_kf: hiddenswitch_comfy.hooks.HookKeyframeGroup=None):
        if prev_hook_kf is None:
            prev_hook_kf = hiddenswitch_comfy.hooks.HookKeyframeGroup()
        prev_hook_kf = prev_hook_kf.clone()
        keyframe = hiddenswitch_comfy.hooks.HookKeyframe(strength=strength_mult, start_percent=start_percent)
...
class CreateHookKeyframesInterpolated:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "interpolation": (hiddenswitch_comfy.hooks.InterpolationMethod._LIST, ),
                "start_percent": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.001}),
                "end_percent": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.001}),
    ...
    def create_hook_keyframes(self, strength_start: float, strength_end: float, interpolation: str,
                              ...
                              print_keyframes=False, prev_hook_kf: hiddenswitch_comfy.hooks.HookKeyframeGroup=None):
        if prev_hook_kf is None:
            prev_hook_kf = hiddenswitch_comfy.hooks.HookKeyframeGroup()
        prev_hook_kf = prev_hook_kf.clone()
        percents = hiddenswitch_comfy.hooks.InterpolationMethod.get_weights(num_from=start_percent, num_to=end_percent, length=keyframes_count,
                                                               method=hiddenswitch_comfy.hooks.InterpolationMethod.LINEAR)
        strengths = hiddenswitch_comfy.hooks.InterpolationMethod.get_weights(num_from=strength_start, num_to=strength_end, length=keyframes_count, method=interpolation)
        ...
        for percent, strength in zip(percents, strengths):
            ...
            if is_first:
                guarantee_steps = 1
                is_first = False
            prev_hook_kf.add(hiddenswitch_comfy.hooks.HookKeyframe(strength=strength, start_percent=percent, guarantee_steps=guarantee_steps))
            if print_keyframes:
...
class CreateHookKeyframesFromFloats:
    ...
    def create_hook_keyframes(self, floats_strength: Union[float, list[float]],
                              ...
                              prev_hook_kf: hiddenswitch_comfy.hooks.HookKeyframeGroup=None, print_keyframes=False):
        if prev_hook_kf is None:
            prev_hook_kf = hiddenswitch_comfy.hooks.HookKeyframeGroup()
        prev_hook_kf = prev_hook_kf.clone()
        ...
        percents = hiddenswitch_comfy.hooks.InterpolationMethod.get_weights(num_from=start_percent, num_to=end_percent, length=len(floats_strength),
                                                               method=hiddenswitch_comfy.hooks.InterpolationMethod.LINEAR)
...
class SetModelHooksOnCond:
    ...
    def attach_hook(self, conditioning, hooks: hiddenswitch_comfy.hooks.HookGroup):
        return (hiddenswitch_comfy.hooks.set_hooks_for_conditioning(conditioning, hooks),)
...
class CombineHooks:
    ...
    def combine_hooks(self,
                      hooks_A: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_B: hiddenswitch_comfy.hooks.HookGroup=None):
        candidates = [hooks_A, hooks_B]
        return (hiddenswitch_comfy.hooks.HookGroup.combine_all_hooks(candidates),)
...
class CombineHooksFour:
    ...
    def combine_hooks(self,
                      hooks_A: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_B: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_C: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_D: hiddenswitch_comfy.hooks.HookGroup=None):
...
class CombineHooksEight:
    ...
    def combine_hooks(self,
                      ...
                      hooks_C: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_D: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_E: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_F: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_G: hiddenswitch_comfy.hooks.HookGroup=None,
                      hooks_H: hiddenswitch_comfy.hooks.HookGroup=None):
